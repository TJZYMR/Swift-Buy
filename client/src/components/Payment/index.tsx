  ...
  useEffect(()=>{
    setTimeout(() => {
        if(webcamRef && webcamRef.current){
            ...
            .then( (response)=> {
              // handle success
              let amount = localStorage.getItem('priceTotal') || "0";
              let c_id = localStorage.getItem('consumer_id') || "1"
              if(response.status === 200 && response.data.success){
              console.log(response.data);
              axios.post(`http://20.20.1.152:5000/payment?consumer_id=${c_id}&amount=${amount}`)
              .then((response)=>{
  ...
  return (
        <div className='container'>
           <h2 className='title'>Payment</h2>
            <span className='scan-text'>Scan to Payment</span>
